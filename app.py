...
if mode == "2D Multi-Stream (Audit Mode)":
    ...
    # Auto-check and download YOLOv11-nano & YOLO-Pose ONNX if missing
    try:
        from utils.ensure_yolo11n import ensure_yolo11n_onnx
        from utils.ensure_yolo_pose import ensure_yolo_pose_onnx
        ensure_yolo11n_onnx()
        ensure_yolo_pose_onnx()
    except Exception as e:
        logging.error(f"Error ensuring ONNX models: {e}")
